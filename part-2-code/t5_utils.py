...
def initialize_model(args):
    '''
    Helper function to initialize the model. Either finetune the pretrained model
    or train from scratch with the T5-small config.
    '''
    if args.finetune:
        ...
        # Initialize T5-small from scratch (random weights with T5-small architecture)
...
def initialize_optimizer(args, model):
    '''
    Initialize AdamW optimizer with weight decay for non-bias and non-LayerNorm parameters
    '''
...
def initialize_scheduler(args, optimizer, epoch_length):
    '''
    Initialize learning rate scheduler (cosine, linear, or none)
    '''
...
def get_parameter_names(model, forbidden_layer_types):
    '''
    Get names of all parameters, excluding those from forbidden layer types
    '''
    result = []
