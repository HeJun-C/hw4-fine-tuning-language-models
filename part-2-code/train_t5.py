...
def eval_epoch(args, model, dev_loader, gt_sql_path, model_sql_path, gt_record_path, model_record_path):
    '''
    Evaluation loop during training. Computes loss and generates SQL queries to evaluate metrics.
    '''
    model.eval()
    total_loss = 0
    total_tokens = 0
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, decoder_input, decoder_targets, initial_decoder_input in tqdm(dev_loader):
            ...
            # Generate SQL queries for metrics
            ...
            batch_sqls = model.config.tokenizer.batch_decode(generated_ids, skip_special_tokens=True)
...
def test_inference(args, model, test_loader, model_sql_path, model_record_path):
    '''
    Inference on test set to generate SQL queries and database records.
    '''
    model.eval()
    generated_sqls = []
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, initial_decoder_input in tqdm(test_loader):
            ...
            batch_sqls = tokenizer.batch_decode(generated_ids, skip_special_tokens=True)
            generated_sqls.extend(batch_sqls)
    
    # Save generated queries and records
    save_queries_and_records(generated_sqls, model_sql_path, model_record_path)
    print(f"Test inference complete. Saved {len(generated_sqls)} SQL queries to {model_sql_path}")
...
def main():
    ...
    # Store tokenizer in model config for decoding
    from transformers import T5TokenizerFast
    model.config.tokenizer = T5TokenizerFast.from_pretrained('google-t5/t5-small')
    ...
    dev_loss, record_f1, record_em, sql_em, dev_error_rate = eval_epoch(args, model, dev_loader,
                                                                        gt_sql_path, model_sql_path,
                                                                        gt_record_path, model_record_path)
    print(f"Dev set results: Loss: {dev_loss:.4f}, Record F1: {record_f1:.4f}, Record EM: {record_em:.4f}, SQL EM: {sql_em:.4f}")
